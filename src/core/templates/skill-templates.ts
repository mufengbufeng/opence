...
/**
 * Template for opence-explore skill
...
You have full context of the opence system. Use it naturally, don't force it.
...
1. **Read existing artifacts for context**
   - \`opence/changes/<name>/proposal.md\`
   - \`opence/changes/<name>/design.md\`
   - \`opence/changes/<name>/tasks.md\`
   - etc.
...
/**
 * Template for opence-new-change skill
 * Based on /opsx:new command
 */
export function getNewChangeSkillTemplate(): SkillTemplate {
  return {
    name: 'opence-new-change',
    description: 'Start a new opence change using the experimental artifact workflow. Use when the user wants to create a new feature, fix, or modification with a structured step-by-step approach.',
...
2. **Determine the workflow schema**
   ...
   **Use a different schema only if the user mentions:**
   ...
   - "show workflows" or "what workflows" → run \`opence schemas --json\` and let them choose

   **Otherwise**: Omit \`--schema\` to use the default.

3. **Create the change directory**
   \`\`\`bash
   opence new change "<name>"
   \`\`\`
   Add \`--schema <name>\` only if the user requested a specific workflow.
   This creates a scaffolded change at \`opence/changes/<name>/\` with the selected schema.

4. **Show the artifact status**
   \`\`\`bash
   opence status --change "<name>"
   \`\`\`
...
5. **Get instructions for the first artifact**
   ...
   \`\`\`bash
   opence instructions <first-artifact-id> --change "<name>"
   \`\`\`
...
/**
 * Template for opence-continue-change skill
 * Based on /opsx:continue command
 */
export function getContinueChangeSkillTemplate(): SkillTemplate {
  return {
    name: 'opence-continue-change',
    description: 'Continue working on an opence change by creating the next artifact. Use when the user wants to progress their change, create the next artifact, or continue their workflow.',
...
1. **If no change name provided, prompt for selection**
   ...
   Run \`opence list --json\` to get available changes sorted by most recently modified. Then use the **AskUserQuestion tool** to let the user select which change to work on.
...
2. **Check current status**
   \`\`\`bash
   opence status --change "<name>" --json
   \`\`\`
...
3. **Act based on status**:
   ...
   **If artifacts are ready to create** (status shows artifacts with \`status: "ready"\`):
   ...
   - Get its instructions:
     \`\`\`bash
     opence instructions <artifact-id> --change "<name>" --json
     \`\`\`
...
/**
 * Template for opence-apply-change skill
 * For implementing tasks from a completed (or in-progress) change
 */
export function getApplyChangeSkillTemplate(): SkillTemplate {
  return {
    name: 'opence-apply-change',
    description: 'Implement tasks from an opence change. Use when the user wants to start implementing, continue implementation, or work through tasks.',
    instructions: `Implement tasks from an opence change.
...
1. **If no change name provided, prompt for selection**
   ...
   Run \`opence list --json\` to get available changes. Use the **AskUserQuestion tool** to let the user select.
...
3. **Get apply instructions**
   ...
   \`\`\`bash
   opence instructions apply --change "<name>" --json
   \`\`\`
   ...
   **Handle states:**
   - If \`state: "blocked"\` (missing artifacts): show message, suggest using opence-continue-change
...
/**
 * Template for opence-ff-change skill
 * Fast-forward through artifact creation
 */
export function getFfChangeSkillTemplate(): SkillTemplate {
  return {
    name: 'opence-ff-change',
    description: 'Fast-forward through opence artifact creation. Use when the user wants to quickly create all artifacts needed for implementation without stepping through each one individually.',
...
2. **Create the change directory**
   ...
   opence new change "<name>"
   \`\`\`
   This creates a scaffolded change at \`opence/changes/<name>/\`.
...
4. **Create artifacts in sequence until apply-ready**
   ...
   b. **Continue until all \`applyRequires\` artifacts are complete**
      - After creating each artifact, re-run \`opence status --change "<name>" --json\`
...
- Follow the \`instruction\` field from \`opence instructions\` for each artifact type
...
2. **Find delta specs**
   ...
   Look for delta spec files in \`opence/changes/<name>/specs/*/spec.md\`.
...
3. **For each delta spec, apply changes to main specs**
   ...
   For each capability with a delta spec at \`opence/changes/<name>/specs/<capability>/spec.md\`:

   a. **Read the delta spec** to understand the intended changes

   b. **Read the main spec** at \`opence/specs/<capability>/spec.md\` (may not exist yet)
   ...
   d. **Create new main spec** if capability doesn't exist yet:
      - Create \`opence/specs/<capability>/spec.md\`
...
export function getOpsxApplyCommandTemplate(): CommandTemplate {
  return {
    name: 'OPSX: Apply',
    description: 'Implement tasks from an opence change (Experimental)',
    category: 'Workflow',
    tags: ['workflow', 'artifacts', 'experimental'],
    content: `Implement tasks from an opence change.
...
2. **Check artifact completion status**
   ...
   Run \`opence status --change "<name>" --json\` to check artifact completion.
...
4. **Check if delta specs need syncing**
   ...
   **If delta specs exist, perform a quick sync check:**
   ...
   a. **For each delta spec** at \`opence/changes/<name>/specs/<capability>/spec.md\`:
      - Extract requirement names (lines matching \`### Requirement: <name>\`)
      - Note which sections exist (ADDED, MODIFIED, REMOVED)

   b. **Check corresponding main spec** at \`opence/specs/<capability>/spec.md\`:
   ...
   c. **Report findings:**
      ...
      **If sync needed:**
      ...
      - If user chooses sync, execute /opsx:sync logic (use the opence-sync-specs skill)
...
5. **Perform the archive**
   ...
   \`\`\`bash
   mkdir -p opence/changes/archive
   \`\`\`
   ...
   **Check if target already exists:**
   ...
   \`\`\`bash
   mv opence/changes/<name> opence/changes/archive/YYYY-MM-DD-<name>
   \`\`\`
...
**Change:** <change-name>
**Schema:** <schema-name>
**Archived to:** opence/changes/archive/YYYY-MM-DD-<name>/
...
- Use artifact graph (opence status --json) for completion checking
- Don't block archive on warnings - just inform and confirm
- Preserve .openspec.yaml when moving to archive (it moves with the directory)
- Show clear summary of what happened
- If sync is requested, use opence-sync-specs approach (agent-driven)
...
**Change:** <change-name>
**Target:** opence/changes/archive/YYYY-MM-DD-<name>/
