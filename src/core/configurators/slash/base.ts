...
const ALL_COMMANDS: SlashCommandId[] = ['plan', 'work', 'review', 'compound'];
...
export abstract class SlashCommandConfigurator {
  ...
  async generateAll(projectPath: string, _openceDir: string): Promise<string[]> {
    ...
    for (const target of this.getTargets()) {
      ...
      if (await FileSystemUtils.fileExists(filePath)) {
        ...
        sections.push(`${OPENCE_MARKERS.start}\n${body}\n${OPENCE_MARKERS.end}`);
  ...
  async updateExisting(projectPath: string, _openceDir: string): Promise<string[]> {
  ...
  protected async updateBody(filePath: string, body: string): Promise<void> {
    ...
    const startIndex = content.indexOf(OPENCE_MARKERS.start);
    const endIndex = content.indexOf(OPENCE_MARKERS.end);

    if (startIndex === -1 || endIndex === -1 || endIndex <= startIndex) {
      throw new Error(`Missing opence markers in ${filePath}`);
    }

    const before = content.slice(0, startIndex + OPENCE_MARKERS.start.length);
