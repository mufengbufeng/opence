...
export class ArchiveCommand {
  async execute(
    ...
    const targetPath = '.';
    const changesDir = path.join(targetPath, 'opence', 'changes');
    const archiveDir = path.join(changesDir, 'archive');
    const mainSpecsDir = path.join(targetPath, 'opence', 'specs');

    // Check if changes directory exists
    try {
      await fs.access(changesDir);
    } catch {
      throw new Error("No opence changes directory found. Run 'opence init' first.");
    }
