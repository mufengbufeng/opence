...
export class PowerShellInstaller {
  ...
  getInstallationPath(): string {
    ...
    return path.join(profileDir, 'openceCompletion.ps1');
  }
  ...
  async configureProfile(scriptPath: string): Promise<boolean> {
    ...
    for (const profilePath of profilePaths) {
      try {
        ...
        // Add opence completion configuration with markers
        const openceBlock = [
          '',
        ...
        const newContent = profileContent + openceBlock;
  ...
  async removeProfileConfig(): Promise<boolean> {
    ...
    for (const profilePath of profilePaths) {
      try {
        ...
        // Remove OPENCE:START -> OPENCE:END block
        const startMarker = '# OPENCE:START';
        const endMarker = '# OPENCE:END';
        const startIndex = profileContent.indexOf(startMarker);

        if (startIndex === -1) {
          continue; // No opence block found
        }
