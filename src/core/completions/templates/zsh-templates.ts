...
# Use opence __complete to get available changes
_opence_complete_changes() {
  local -a changes
  while IFS=$'\\t' read -r id desc; do
    changes+=("$id:$desc")
  done < <(opence __complete changes 2>/dev/null)
  _describe "change" changes
}

# Use opence __complete to get available specs
_opence_complete_specs() {
  local -a specs
  while IFS=$'\\t' read -r id desc; do
    specs+=("$id:$desc")
  done < <(opence __complete specs 2>/dev/null)
