...
/**
 * Generates PowerShell completion scripts for the opence CLI.
...
export class PowerShellGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    const commandEntries = commands.map(
      (cmd) => `@{Name="${cmd.name}"; Description="${this.escapeDescription(cmd.description)}"}`
    );
    const commandLines: string[] = [];
    this.appendArrayEntries(commandLines, '            ', commandEntries);
    ...
    return `# PowerShell completion script for opence CLI
...
`;
  ...
  private generateCommandCase(cmd: CommandDefinition, indent: string): string[] {
    ...
    if (cmd.subcommands && cmd.subcommands.length > 0) {
      ...
      if (cmd.flags.length > 0) {
        ...
        const flagEntries = this.buildFlagEntries(cmd.flags);
        this.appendArrayEntries(lines, `${indent}        `, flagEntries);
        lines.push(`${indent}    )`);
      ...
      const subcommandEntries = cmd.subcommands.map(
        (subcmd) => `@{Name="${subcmd.name}"; Description="${this.escapeDescription(subcmd.description)}"}`
      );
      this.appendArrayEntries(lines, `${indent}        `, subcommandEntries);
      lines.push(`${indent}    )`);
  ...
  private generatePositionalCompletion(positionalType: string | undefined, indent: string): string[] {
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`${indent}Get-OpenceChanges | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {`);
        lines.push(`${indent}    [System.Management.Automation.CompletionResult]::new($_, $_, "ParameterValue", "Change: $_")`);
        lines.push(`${indent}}`);
        break;
      case 'spec-id':
        lines.push(`${indent}Get-OpenceSpecs | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {`);
        lines.push(`${indent}    [System.Management.Automation.CompletionResult]::new($_, $_, "ParameterValue", "Spec: $_")`);
        lines.push(`${indent}}`);
        break;
      case 'change-or-spec-id':
        lines.push(`${indent}$items = @(Get-OpenceChanges) + @(Get-OpenceSpecs)`);
  ...
  private buildFlagEntries(flags: FlagDefinition[]): string[] {
    ...
    for (const flag of flags) {
      ...
      if (shortFlag) {
        entries.push(`@{Name="${longFlag}"; Description="${description}"}`);
        entries.push(`@{Name="${shortFlag}"; Description="${description}"}`);
      } else {
        entries.push(`@{Name="${longFlag}"; Description="${description}"}`);
      }
    }
  ...
  private appendArrayEntries(lines: string[], indent: string, entries: string[]): void {
