...
/**
 * Generates Bash completion scripts for the opence CLI.
...
export class BashGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    return `# Bash completion script for opence CLI
...
${helpers}
complete -F _opence_completion opence
`;
  }
  ...
  private generatePositionalCompletion(positionalType: string | undefined, indent: string): string[] {
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`${indent}_opence_complete_changes`);
        break;
      case 'spec-id':
        lines.push(`${indent}_opence_complete_specs`);
        break;
      case 'change-or-spec-id':
        lines.push(`${indent}_opence_complete_items`);
        break;
      case 'shell':
