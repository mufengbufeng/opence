...
/**
 * Generates Zsh completion scripts for the opence CLI.
 * Follows Zsh completion system conventions using the _opence function.
 */
export class ZshGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    for (const cmd of commands) {
      ...
      commandCaseLines.push(`          _opence_${this.sanitizeFunctionName(cmd.name)}`);
    ...
    return `#compdef opence

# Zsh completion script for opence CLI
...
${commandList}
  ...
  case $state in
    command)
      _describe "opence command" commands
      ;;
    args)
      case $words[1] in
${commandCases}
...
`;
  ...
  private generateCommandFunction(cmd: CommandDefinition): string[] {
    const funcName = `_opence_${this.sanitizeFunctionName(cmd.name)}`;
    const lines: string[] = [];
    ...
    if (cmd.subcommands && cmd.subcommands.length > 0) {
      ...
      for (const subcmd of cmd.subcommands) {
        ...
        lines.push(`          _opence_${this.sanitizeFunctionName(cmd.name)}_${this.sanitizeFunctionName(subcmd.name)}`);
        lines.push('          ;;');
      }
  ...
  private generateSubcommandFunction(parentName: string, subcmd: CommandDefinition): string[] {
    const funcName = `_opence_${this.sanitizeFunctionName(parentName)}_${this.sanitizeFunctionName(subcmd.name)}`;
    const lines: string[] = [];
  ...
  private generatePositionalSpec(positionalType?: string): string {
    switch (positionalType) {
      case 'change-id':
        return "'*: :_opence_complete_changes'";
      case 'spec-id':
        return "'*: :_opence_complete_specs'";
      case 'change-or-spec-id':
        return "'*: :_opence_complete_items'";
      case 'path':
        return "'*:path:_files'";
      case 'shell':
