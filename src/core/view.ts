...
export class ViewCommand {
  async execute(targetPath: string = '.'): Promise<void> {
    const openceDir = path.join(targetPath, 'opence');
    
    if (!fs.existsSync(openceDir)) {
      console.error(chalk.red('No opence directory found'));
      process.exit(1);
    }
    ...
    const changesData = await this.getChangesData(openceDir);
    const specsData = await this.getSpecsData(openceDir);
    ...
    console.log('\n' + '═'.repeat(60));
    console.log(chalk.dim(`\nUse ${chalk.white('opence list --changes')} or ${chalk.white('opence list --specs')} for detailed views`));
  }

  private async getChangesData(openceDir: string): Promise<{
    draft: Array<{ name: string }>;
    active: Array<{ name: string; progress: { total: number; completed: number } }>;
    completed: Array<{ name: string }>;
  }> {
    const changesDir = path.join(openceDir, 'changes');
  ...
  private async getSpecsData(openceDir: string): Promise<Array<{ name: string; requirementCount: number }>> {
    const specsDir = path.join(openceDir, 'specs');
