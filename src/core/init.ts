  ...
  (config, done) => {
    ...
    if (step === 'intro') {
      const introHeadline = config.extendMode
        ? 'Extend your opence tooling'
        : 'Configure your opence tooling';
      const introBody = config.extendMode
        ? 'We detected an existing setup. We will help you refresh or add integrations.'
        : "Let's get your AI assistants connected so they understand opence.";
...
export class InitCommand {
  ...
  async execute(targetPath: string): Promise<void> {
    ...
    const openceDir = OPENCE_DIR_NAME;
    const opencePath = path.join(projectPath, openceDir);

    // Validation happens silently in the background
    const extendMode = await this.validate(projectPath, opencePath);
    ...
    if (!extendMode) {
      const structureSpinner = this.startSpinner(
        'Creating opence structure...'
      );
      await this.createDirectoryStructure(opencePath);
      await this.generateFiles(opencePath, config);
      structureSpinner.stopAndPersist({
        symbol: PALETTE.white('▌'),
        text: PALETTE.white('opence structure created'),
      });
    } else {
      ora({ stream: process.stdout }).info(
        PALETTE.midGray(
          'ℹ opence already initialized. Checking for missing files...'
        )
      );
      await this.createDirectoryStructure(opencePath);
      await this.ensureTemplateFiles(opencePath, config);
    }
  ...
  private async validate(
    ...
    const extendMode = await FileSystemUtils.directoryExists(_opencePath);
    ...
    const choices: ToolWizardChoice[] = [
      {
        ...
        label: {
          primary:
            'Natively supported providers (✔ opence custom slash commands available)',
        },
        selectable: false,
      },
        ...
        : []),
      {
        ...
        label: {
          primary:
            'Other tools (use Universal AGENTS.md for Amp, VS Code, and others)',
        },
        selectable: false,
      },
      {
  ...
  private async isToolConfigured(
    ...
    // A tool is only considered "configured by opence" if its files contain opence markers.
    ...
    // Helper to check if a file exists and contains opence markers
    const fileHasMarkers = async (absolutePath: string): Promise<boolean> => {
      try {
        const content = await FileSystemUtils.readFile(absolutePath);
        return content.includes(OPENCE_MARKERS.start) && content.includes(OPENCE_MARKERS.end);
      } catch {
        return false;
      }
    };
    ...
    // Check if the tool has a config file with opence markers
    ...
    // Check if any slash command file exists with opence markers
  ...
  private async createDirectoryStructure(opencePath: string): Promise<void> {
    const directories = [
      opencePath,
      path.join(opencePath, 'specs'),
      path.join(opencePath, 'changes'),
      path.join(opencePath, 'changes', 'archive'),
    ];
  ...
  private async generateFiles(
    ...
    await this.writeTemplateFiles(opencePath, config, false);
  }
  ...
  private async ensureTemplateFiles(
    ...
    await this.writeTemplateFiles(opencePath, config, true);
  }
  ...
  private async writeTemplateFiles(
    ...
    for (const template of templates) {
      const filePath = path.join(opencePath, template.path);
  ...
  private async configureAITools(
    ...
    for (const toolId of toolIds) {
      ...
      if (configurator && configurator.isAvailable) {
        await configurator.configure(projectPath, openceDir);
      }

      const slashConfigurator = SlashCommandRegistry.get(toolId);
      if (slashConfigurator && slashConfigurator.isAvailable) {
        await slashConfigurator.generateAll(projectPath, openceDir);
      }
    }

    await this.configureSkills(projectPath, toolIds);

    return rootStubStatus;
  }

  private async configureSkills(
    projectPath: string,
    toolIds: string[]
  ): Promise<void> {
    ...
    if (wantsClaudeSkills) {
      await this.writeSkillSet(projectPath, 'claude');
    }

    if (wantsCodexSkills) {
      await this.writeSkillSet(projectPath, 'codex');
    }
  }

  private async writeSkillSet(
    projectPath: string,
    ...
    for (const id of OPENCE_SKILL_IDS) {
      ...
      if (await FileSystemUtils.fileExists(skillFile)) {
        ...
        const updated = updateOpenceSkillContent(existing, id, variant);
        await FileSystemUtils.writeFile(skillFile, updated);
      } else {
  ...
  private displaySuccessMessage(
    ...
    const successHeadline = extendMode
      ? 'opence tool configuration updated!'
      : 'opence initialized successfully!';
    ...
    console.log(
      PALETTE.midGray(
        'Use `opence update` to refresh shared opence instructions in the future.'
      )
    );
    ...
    if (created.length > 0 || refreshed.length > 0) {
      ...
      console.log(
        PALETTE.midGray(
          'to ensure the new /opence commands appear in your command palette.'
        )
      );
    }
    ...
    console.log(
      PALETTE.lightGray(
        '   "Please read opence/project.md and help me fill it out'
      )
    );
    console.log(
      PALETTE.lightGray(
    ...
    console.log(
      PALETTE.lightGray('    opence change proposal for this feature"\n')
    );
    console.log(PALETTE.white('3. Learn the opence workflow:'));
    console.log(
      PALETTE.lightGray(
        '   "Please explain the opence workflow from opence/AGENTS.md'
      )
    );
    console.log(
  ...
  private renderBanner(_extendMode: boolean): void {
    ...
    console.log();
    console.log(PALETTE.white('Welcome to opence!'));
    console.log();
  }
