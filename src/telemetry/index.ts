 ...
 * - Opt-out via OPENCE_TELEMETRY=0 or DO_NOT_TRACK=1
...
const POSTHOG_HOST = 'https://edge.opence.dev';
...
export function isTelemetryEnabled(): boolean {
  ...
  if (process.env.OPENCE_TELEMETRY === '0') {
    return false;
  }
 ...
 * @param version - The opence version
 */
...
export async function maybeShowTelemetryNotice(): Promise<void> {
  ...
  try {
    ...
    // Display notice
    console.log(
      'Note: opence collects anonymous usage stats. Opt out: OPENCE_TELEMETRY=0'
    );
