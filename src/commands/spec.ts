...
export class SpecCommand {
  private SPECS_DIR = 'opence/specs';
  ...
  async show(specId?: string, options: ShowOptions = {}): Promise<void> {
    ...
    if (!existsSync(specPath)) {
      throw new Error(`Spec '${specId}' not found at opence/specs/${specId}/spec.md`);
    }
...
export function registerSpecCommand(rootProgram: typeof program) {
  const specCommand = rootProgram
    .command('spec')
    .description('Manage and view opence specifications');

  // Deprecation notice for noun-based commands
  specCommand.hook('preAction', () => {
    console.error('Warning: The "opence spec ..." commands are deprecated. Prefer verb-first commands (e.g., "opence show", "opence validate --specs").');
  });
