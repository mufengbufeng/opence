...
async function validateChangeExists(
  ...
  const changesPath = path.join(projectRoot, 'opence', 'changes');
  ...
  if (!changeName) {
    ...
    if (available.length === 0) {
      throw new Error('No changes found. Create one with: opence new change <name>');
    }
    throw new Error(
  ...
  if (!exists) {
    ...
    if (available.length === 0) {
      throw new Error(
        `Change '${changeName}' not found. No changes exist. Create one with: opence new change <name>`
      );
    }
    throw new Error(
...
async function generateApplyInstructions(
  ...
  const changeDir = path.join(projectRoot, 'opence', 'changes', changeName);
  ...
  if (missingArtifacts.length > 0) {
    state = 'blocked';
    instruction = `Cannot apply this change yet. Missing artifacts: ${missingArtifacts.join(', ')}.\nUse the opence-continue-change skill to create the missing artifacts first.`;
  } else if (tracksFile && !tracksFileExists) {
    // Tracking file configured but doesn't exist yet
    const tracksFilename = path.basename(tracksFile);
    state = 'blocked';
    instruction = `The ${tracksFilename} file is missing and must be created.\nUse opence-continue-change to generate the tracking file.`;
  } else if (tracksFile && tracksFileExists && total === 0) {
    // Tracking file exists but contains no tasks
    const tracksFilename = path.basename(tracksFile);
    state = 'blocked';
    instruction = `The ${tracksFilename} file exists but contains no tasks.\nAdd tasks to ${tracksFilename} or regenerate it with opence-continue-change.`;
...
function printApplyInstructionsText(instructions: ApplyInstructions): void {
  ...
  if (state === 'blocked' && missingArtifacts) {
    ...
    console.log('Use the opence-continue-change skill to create these first.');
    console.log();
  }
...
async function newChangeCommand(name: string | undefined, options: NewChangeOptions): Promise<void> {
  ...
  try {
    ...
    if (options.description) {
      ...
      const changeDir = path.join(projectRoot, 'opence', 'changes', name);
      const readmePath = path.join(changeDir, 'README.md');
      await fs.writeFile(readmePath, `# ${name}\n\n${options.description}\n`, 'utf-8');
    }

    const schemaUsed = options.schema ?? DEFAULT_SCHEMA;
    spinner.succeed(`Created change '${name}' at opence/changes/${name}/ (schema: ${schemaUsed})`);
  } catch (error) {
...
async function artifactExperimentalSetupCommand(): Promise<void> {
  ...
  try {
    ...
    const skills = [
      { template: exploreSkill, dirName: 'opence-explore' },
      { template: newChangeSkill, dirName: 'opence-new-change' },
      { template: continueChangeSkill, dirName: 'opence-continue-change' },
      { template: applyChangeSkill, dirName: 'opence-apply-change' },
      { template: ffChangeSkill, dirName: 'opence-ff-change' },
      { template: syncSpecsSkill, dirName: 'opence-sync-specs' },
      { template: archiveChangeSkill, dirName: 'opence-archive-change' },
    ];
...
`;
    ...
    console.log('  • "I want to start a new opence change to add <feature>"');
    ...
    console.log('   Feedback welcome at: https://github.com/Fission-AI/opence/issues');
    console.log();
  } catch (error) {
