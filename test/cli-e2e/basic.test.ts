...
async function prepareFixture(fixtureName: string): Promise<string> {
  const base = await fs.mkdtemp(path.join(tmpdir(), 'opence-cli-e2e-'));
  tempRoots.push(base);
...
describe('opence CLI e2e basics', () => {
  it('shows help output', async () => {
    const result = await runCLI(['--help']);
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('Usage: opence');
  ...
  describe('init command non-interactive options', () => {
    it('initializes with --tools all option', async () => {
      ...
      const claudePlan = path.join(emptyProjectDir, '.claude/commands/opence/plan.md');
      const copilotPlan = path.join(emptyProjectDir, '.github/prompts/opence-plan.prompt.md');
      const codexPlan = path.join(codexHome, 'prompts', 'opence-plan.md');

      expect(await fileExists(claudePath)).toBe(true);
      expect(await fileExists(claudePlan)).toBe(true);
      expect(await fileExists(copilotPlan)).toBe(true);
      expect(await fileExists(codexPlan)).toBe(true);
    });
    ...
    it('initializes with --tools list option', async () => {
      ...
      expect(await fileExists(claudePath)).toBe(true);
      expect(await fileExists(claudePlan)).toBe(true);
      expect(await fileExists(copilotPlan)).toBe(false); // Not selected
      expect(await fileExists(codexPlan)).toBe(false); // Not selected
    });
    ...
    it('initializes with --tools none option', async () => {
      ...
      expect(await fileExists(claudePlan)).toBe(false);
      expect(await fileExists(copilotPlan)).toBe(false);
      expect(await fileExists(codexPlan)).toBe(false);
    });
