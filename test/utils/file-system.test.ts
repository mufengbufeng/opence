...
describe('FileSystemUtils', () => {
  ...
  describe('joinPath', () => {
    it('should join POSIX-style paths', () => {
      ...
      expect(result).toBe('/tmp/project/.claude/commands/opence/plan.md');
    });

    it('should join Linux home directory paths', () => {
      const result = FileSystemUtils.joinPath(
        '/home/dev/workspace/opence',
        '.github/prompts/opence-plan.prompt.md'
      );
      expect(result).toBe('/home/dev/workspace/opence/.github/prompts/opence-plan.prompt.md');
    });
    ...
    it('should join Windows drive-letter paths with backslashes', () => {
      ...
      expect(result).toBe(
        'C:\\Users\\dev\\project\\.claude\\commands\\opence\\plan.md'
      );
    });
    ...
    it('should join Windows paths that use forward slashes', () => {
      ...
      expect(result).toBe(
        'D:\\workspace\\app\\.github\\prompts\\opence-work.prompt.md'
      );
    });
    ...
    it('should join UNC-style Windows paths', () => {
      ...
      expect(result).toBe(
        '\\server\\share\\repo\\.github\\prompts\\opence-compound.prompt.md'
      );
    });
  });
});
