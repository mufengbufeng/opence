...
describe('telemetry/index', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `opence-telemetry-test-${randomUUID()}`);
  ...
  describe('isTelemetryEnabled', () => {
    it('should return false when OPENCE_TELEMETRY=0', () => {
      process.env.OPENCE_TELEMETRY = '0';
    ...
    it('should return true when no opt-out is set', () => {
      delete process.env.OPENCE_TELEMETRY;
      delete process.env.DO_NOT_TRACK;
      delete process.env.CI;
      expect(isTelemetryEnabled()).toBe(true);
    });

    it('should prioritize OPENCE_TELEMETRY=0 over other settings', () => {
      process.env.OPENCE_TELEMETRY = '0';
      delete process.env.DO_NOT_TRACK;
      delete process.env.CI;
      expect(isTelemetryEnabled()).toBe(false);
