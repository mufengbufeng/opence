...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for opence CLI');
			expect(script).toContain('$openceCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for opence command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize opence',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName opence');
			expect(script).toContain('-ScriptBlock $openceCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-OpenceChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-OpenceSpecs');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-OpenceChanges');
			expect(script).toContain('opence __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-OpenceSpecs');
			expect(script).toContain('opence __complete specs 2>$null');
		});
