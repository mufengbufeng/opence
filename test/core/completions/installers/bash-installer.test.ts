...
describe('BashInstaller', () => {
  ...
  beforeEach(async () => {
    ...
    testHomeDir = path.join(os.tmpdir(), `opence-bash-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return standard bash-completion path', async () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.local', 'share', 'bash-completion', 'completions', 'opence'));
    });
  });
  ...
  describe('install', () => {
    const testScript = '# Bash completion script for opence CLI\n_opence_completion() {\n  echo "test"\n}\n';

    it('should install to bash-completion path', async () => {
      const result = await installer.install(testScript);

      expect(result.success).toBe(true);
      expect(result.installedPath).toBe(path.join(testHomeDir, '.local', 'share', 'bash-completion', 'completions', 'opence'));
    ...
    it('should backup existing file before overwriting', async () => {
      const targetPath = path.join(testHomeDir, '.local', 'share', 'bash-completion', 'completions', 'opence');
    ...
    it('should handle paths with spaces in .bashrc config', async () => {
      ...
      const testHomeDirWithSpaces = path.join(os.tmpdir(), `opence bash test ${randomUUID()}`);
  ...
  describe('uninstall', () => {
    ...
    it('should remove .bashrc configuration', async () => {
      ...
      if (exists) {
        ...
        expect(content).not.toContain('# OPENCE:END');
      }
    });
  });
  ...
  describe('removeBashrcConfig', () => {
    ...
    it('should remove markers and config when present', async () => {
      ...
      const content = [
        ...
        '  . ~/.local/share/bash-completion/completions/opence',
        'fi',
        '# OPENCE:END',
        '',
        'alias ll="ls -la"',
      ...
      expect(newContent).not.toContain('opence shell completions configuration');
