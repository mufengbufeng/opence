...
describe('ListCommand', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(os.tmpdir(), `opence-list-test-${Date.now()}`);
  ...
  describe('execute', () => {
    it('should handle missing opence/changes directory', async () => {
      const listCommand = new ListCommand();
      
      await expect(listCommand.execute(tempDir, 'changes')).rejects.toThrow(
        "No opence changes directory found. Run 'opence init' first."
      );
    });

    it('should handle empty changes directory', async () => {
      const changesDir = path.join(tempDir, 'opence', 'changes');
