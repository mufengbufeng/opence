...
describe('artifact-graph/resolver', () => {
  ...
  beforeEach(() => {
    tempDir = path.join(os.tmpdir(), `opence-resolver-test-${Date.now()}`);
  ...
  describe('getUserSchemasDir', () => {
    it('should use XDG_DATA_HOME when set', () => {
      ...
      expect(userDir).toBe(path.join(tempDir, 'opence', 'schemas'));
    });
  });
  ...
  describe('getSchemaDir', () => {
    ...
    it('should prefer user override directory', () => {
      process.env.XDG_DATA_HOME = tempDir;
      const userSchemaDir = path.join(tempDir, 'opence', 'schemas', 'spec-driven');
...
`;
  ...
  describe('listSchemas', () => {
    ...
    it('should include user override schemas', () => {
      process.env.XDG_DATA_HOME = tempDir;
      const userSchemaDir = path.join(tempDir, 'opence', 'schemas', 'custom-workflow');
      fs.mkdirSync(userSchemaDir, { recursive: true });
    ...
    it('should only include directories with schema.yaml', () => {
      process.env.XDG_DATA_HOME = tempDir;
      const userSchemasBase = path.join(tempDir, 'opence', 'schemas');
